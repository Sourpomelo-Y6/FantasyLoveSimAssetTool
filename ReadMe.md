...
- 音声タブ: Unityプロジェクト内のBGM・SE・VOICEの導入状況、参照数、期待パスを確認

### 音声ライブラリの確認

`音声` タブで最初に `選択` を押し、対象Unityプロジェクトの
`ProjectSettings/ProjectVersion.txt` を選択します。設定は端末ローカルへ保存され、
キャラクターデータやExport JSONには絶対パスを書き込みません。
...
カテゴリ、未配置、文字列検索、選択中キャラクターのVOICEに絞り込めます。一覧で
