...
- 状況テンプレートごとの推奨条件確認と、明示した項目だけを反映する入力補助
- 保存済み会話用固有プロンプトがない場合の、基本情報からの一時構築・確認・明示保存
- 基本情報の「会話AI設定」に集約した、キャラクター概要・口調・関係性・感情・禁止表現・参考台詞の編集と未保存管理
- 会話AI設定の未保存表示、キャラクター切替・再読込・保存先変更・終了時の保存確認、直前バックアップの復元
