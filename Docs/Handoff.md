...
- 画像用途別フォルダ作成
- 画像登録と用途別フォルダへのコピー
- 登録済み画像の登録解除。`profile.json` から外すだけで、画像ファイルと prompt JSON は削除しない
- 登録解除後に同じ `AssetId` で再登録する場合、保存先画像ファイルが残っていれば上書き確認を表示する
...
- ComfyUI WebSocket からの生成中 node と sampler step の詳細進捗表示。接続失敗時は `/queue` と `/history` の確認を継続する
...
- ヒロイン画像タブでの通常、攻撃、被ダメージ、勝利、敗北の戦闘画像標準候補追加
...
- 画像タブ: 画像登録、登録解除、ドラッグ&ドロップ入力、上書き確認、ステータス編集、プレビュー
- スチル作業タブ: 通常作業の主導線。用途フィルタ付きの作業リスト、状況表示、画像プレビュー、合成 prompt、Prompt 反映、画像登録欄への反映、スチル保存
- スチル一覧(確認用)タブ: 仕様上必要なスチルの表形式一覧、状態と追加 prompt の編集。全体確認や開発確認用として残す
...
40. スチル作業タブを通常作業の主導線として明示し、スチル一覧タブを確認用としてリネームする
...
49. Export 時に `ConversationEntries` から `conversations_export.json`、`game_events_export.json`、`action_reactions_export.json`、`endings_export.json` を出力する。ゲームイベントの `affectionChange` は「イベント完了時変化」として保存し、Unityとの往復で維持する。制作状況のイベント詳細には符号付きで表示し、`-9999〜9999` の範囲外は制作状況とExport前検証でエラーにする。発火種類と対象IDは `conditions.triggerType` / `triggerContextId` で往復し、コンテキスト発火なのに対象IDが空ならエラーにする
...
現在の優先順位、保留、見送りは [`ProjectStatus.md`](ProjectStatus.md) を正本とする。この節にあった初期実装時の候補は、その後実装済みになった項目と現在も判断保留の項目が混在していたため集約した。

新しい作業は、実制作を止めている不具合、制作状況で確認できる実データ不足、繰り返し操作の負担の順に選ぶ。過去の実装順序は上の節へ履歴として残す。
...
次に優先するなら、スチル一覧(確認用)タブをスチル作業タブへ完全統合するかの判断、または登録済み画像の差し替え導線を進めるとよいです。
...
敵キャラ素材については、`EnemyProfile`、`EnemyAsset`、`EnemyProjectService`、`EnemyExportService`、最小の「敵キャラ素材」タブまで追加済みです。
