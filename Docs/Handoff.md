...
49. Export 時に `ConversationEntries` から `conversations_export.json`、`game_events_export.json`、`action_reactions_export.json`、`endings_export.json` を出力する。ゲームイベントの `affectionChange` は「イベント完了時変化」として保存し、Unityとの往復で維持する。制作状況のイベント詳細には符号付きで表示し、`-9999〜9999` の範囲外は制作状況とExport前検証でエラーにする
