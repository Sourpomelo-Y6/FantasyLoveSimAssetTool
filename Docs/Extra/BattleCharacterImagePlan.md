...
プレイヤー画像もヒロインとは別の共通素材として扱う。

## 目的

- 戦闘 UI で表示するヒロイン立ち絵、ダメージ差分、勝敗差分を Tool 側で管理できるようにする。
- 戦闘 UI で表示するプレイヤー立ち絵、攻撃差分、被ダメージ差分、勝敗差分を Tool 側で管理できるようにする。
...
- プレイヤー画像は `Assets/Images/Player/Battle/` へ分ける。
...
| プレイヤー通常表示 | `Battle_Player_Idle` | 最初に必要 |
| プレイヤー攻撃 | `Battle_Player_Attack` | コマンド演出用 |
| プレイヤー被ダメージ | `Battle_Player_Damage` | ダメージ演出用 |
| プレイヤー勝利 | `Battle_Player_Victory` | 勝利結果用 |
| プレイヤー敗北 | `Battle_Player_Defeat` | 敗北結果用 |
...
| 敵撃破 | `Enemy_<EnemyId>_Defeat` | enemy export 側で扱う |
...
```text
Images/Battle/Battle_Player_Idle.png
Images/Battle/Battle_Player_Attack.png
Images/Battle/Battle_Player_Damage.png
Images/Battle/Battle_Player_Victory.png
Images/Battle/Battle_Player_Defeat.png
```
...
- プレイヤー戦闘画像: `1024x1024` または `1024x1536`
- 敵画像: `1024x1024`
- 背景透過 PNG
...
本格的に増やす前に、BattlePanel の左右配置と味方側表示確認用として次を用意する。
...
これで BattlePanel にプレイヤー側 Image、ヒロイン側 Image、敵側 Image を追加したとき、表示・サイズ・アンカー・左右配置を確認できる。
