...
8. Unityの訓練カタログは一回限定、調子条件、前提訓練、表示規則を出力し、前提完了で将来解放される訓練も制作対象へ含める。AssetTool側は条件バッジ、詳細表示、前提訓練・解放ノードの参照警告、自己参照・循環参照の検出、制作状況からの該当訓練選択に対応済み。選択中の訓練は回数、表示・実行可能な調子、前提条件、表示規則、解放NodeIdを編集でき、`training_catalog_export.json`からUnityへ戻す。共通`TrainingData`の条件は全ヒロインへ影響するため、ヒロイン固有の解放経路は対象ヒロインの`SkillTreeNodeData`で分ける。
