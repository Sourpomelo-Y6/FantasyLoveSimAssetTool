...
訓練画像タブのAI入力補完は、ヒロインセリフと画像Promptを別機能として段階的に実装する。画像マッピング、AssetId、訓練条件などの構造データはAIへ決定させない。AI入力補助は入力対象と同じ `画像作成・採用` ページへ置き、新しいタブは追加しない。
...
UIは現在の `ヒロインセリフ候補` 内に折り畳み式の `セリフAI` を置く。閉じた状態でも候補数と生成状態を確認できるようにし、セリフ一覧、選択中本文、Voice IDと同じ場所で比較・採用できるようにする。

### 第2段階: 画像positive prompt

セリフ生成が安定した後、選択中の訓練画像に対するpositive promptの補完を追加する。完成Prompt全体をAIへ作り直させるのではなく、現在のPromptへ加える差分タグを最大3案生成する。文脈には現在のpositive prompt、訓練ID・表示名、表示状態、主人公・ヒロインの表示有無、ヒロインの外見に必要な短い情報、ユーザーのキーワードまたは構図案を使う。

生成対象はポーズ、表情、視線、構図、カメラ距離、疲労・動作表現、背景、小物とし、ComfyUIで扱いやすい英語のカンマ区切りタグに限定する。候補の採用は `末尾へ追加` と `現在値と置き換え` を別操作にし、採用前のPromptを保持する。UIはpositive prompt欄の直下に折り畳み式の `画像Prompt AI` として置く。
...
1. 訓練セリフ用の生成コンテキスト、生成サービス、生成元追跡セッションを作成する。
2. `セリフAI` の追加プロンプト、3案生成、警告、新規追加・置き換えを実装する。
3. プロンプト構築、JSON解析、候補採用、誤反映防止の単体テストを追加する。
4. 固定5状態で実データを使い、話者、内容、状態の取り違えがないことを確認する。
5. 画像Promptの差分候補生成と、追加・置き換え操作を実装する。
6. ComfyUIへ送るPromptと保存済みprompt JSONの往復をテストする。
